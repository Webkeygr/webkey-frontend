...
/** Simplex-like 3D noise (inline, χωρίς dependency) */
function createSimplex3D() {
  ...
  return (xin: number, yin: number, zin: number) => {
    ...
    if (t0 > 0) {
      t0 *= t0;
      n0 =
        t0 * t0 * dot(grad3[perm[ii + perm[jj + perm[kk]]] % 12], x0, y0, z0);
    }
    ...
    if (t3 > 0) {
      ...
      n3 =
        t3 *
        t3 *
        dot(grad3[perm[ii + 1 + perm[jj + 1 + perm[kk + 1]]] % 12], x3, y3, z3);
    }
...
export default function Iridescence({
  ...
  const getSpeed = () => {
    if (typeof speed === "number")
      return Math.max(0.0005, Math.min(0.004, 0.002 * speed));
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // feather πάνω
      if (topY > 0) {
        const solidTop = Math.max(0, topY - bandFeatherPx);
        ctx.fillStyle = backgroundFill;
        ctx.fillRect(0, 0, w, solidTop);
        const gt = ctx.createLinearGradient(0, solidTop, 0, topY + 1);
        gt.addColorStop(0, backgroundFill);
        gt.addColorStop(1, "rgba(255,255,255,0)");
        ctx.fillStyle = gt;
        ctx.fillRect(0, solidTop, w, bandFeatherPx + 2);
      }
      // feather κάτω
      if (bottomY < h) {
        const gb = ctx.createLinearGradient(
          ...
          bottomY - 1,
          0,
          bottomY + bandFeatherPx
        );
        gb.addColorStop(0, "rgba(255,255,255,0)");
        gb.addColorStop(1, backgroundFill);
        ctx.fillStyle = gb;
        ctx.fillRect(0, bottomY - 1, w, bandFeatherPx + 2);
